...
public class AlumnosVistaController {
    ...
    public String mostrarPaginaAlumnos(Model model,
                                       ...
                                       @RequestParam(required = false) Long carreraFilter,
                                       @RequestParam(name = "add", required = false) Boolean add) {

        List<AlumnosDto> alumnosFiltrados = alumnosService.buscarPorFiltros(carnetFilter, nombreFilter, emailFilter, gradoFilter, seccionFilter, carreraFilter);
        ...
        model.addAttribute("gradosMap", gradosService.obtenerTodo().stream().collect(Collectors.toMap(GradosDto::idGrado, GradosDto::nombreGrado)));
        model.addAttribute("seccionesMap", seccionesService.obtenerSeccion().stream().collect(Collectors.toMap(SeccionesDto::idSeccion, SeccionesDto::nombreSeccion)));
        model.addAttribute("carrerasMap", carrerasService.obtenerTodo().stream().collect(Collectors.toMap(c -> c.idCarrera().longValue(), CarrerasDto::nombreCarrera)));
    ...
    public String mostrarFormularioEdicion(@PathVariable("id") Long idAlumno, Model model, RedirectAttributes redirectAttributes,
        ...
        try {
            ...
            model.addAttribute("alumnoParaFormulario", alumnosMapper.toEntity(alumnoDto));
        ...
        return mostrarPaginaAlumnos(model, carnetFilter, nombreFilter, emailFilter, gradoFilter, seccionFilter, carreraFilter, null);
    }
    ...
    public String guardarAlumno(@ModelAttribute("alumnoParaFormulario") Alumnos alumnoEntity, RedirectAttributes redirectAttributes) {
        try {
            if (alumnoEntity.getIdAlumno() == null) {
                alumnosService.guardarAlumno(alumnosMapper.toDto(alumnoEntity));
                ...
                ModAlumnosDto modDto = new ModAlumnosDto(alumnoEntity.getCarnetAlumno(), alumnoEntity.getNombreCompleto(), alumnoEntity.getApellidoCompleto(), alumnoEntity.getEmailAcademico(), alumnoEntity.getContrasena(), alumnoEntity.getDireccion(), alumnoEntity.getIdGrado(), alumnoEntity.getIdSeccion(), alumnoEntity.getIdJornada(), alumnoEntity.getIdCarrera(), alumnoEntity.getIdTutor());
            ...
            redirectAttributes.addFlashAttribute("errorMessage", "Error de integridad de datos. Verifique valores duplicados.");
        } catch (Exception e) {
